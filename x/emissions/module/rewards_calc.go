...
import (
	...
	params "github.com/evoluteai-network/evoluteai-chain/app/params"
	state "github.com/evoluteai-network/evoluteai-chain/x/emissions"
	"github.com/evoluteai-network/evoluteai-chain/x/emissions/keeper"
...
func emitRewards(ctx sdk.Context, am AppModule) error {
	...
	emissionsAddress := am.keeper.AccountKeeper().GetModuleAddress(state.evoluteaiRewardsAccountName)
...
func emitRewardsToTopicParticipants(
	...
	for participant, reward := range rewards {
		...
		am.keeper.BankKeeper().SendCoinsFromModuleToModule(ctx, state.evoluteaiRewardsAccountName, state.evoluteaiStakingAccountName, rewardCoins)
	}
	if len(rewards) == 0 {
		fmt.Printf(" No rewards to emit for Topic %v \n", topic)
	}
