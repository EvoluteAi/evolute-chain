...
func (k *Keeper) InitGenesis(ctx context.Context, data *state.GenesisState) error {
	...
	stakingModuleAccount := k.authKeeper.GetModuleAccount(ctx, state.evoluteaiStakingAccountName)
	k.authKeeper.SetModuleAccount(ctx, stakingModuleAccount)
	requestsModuleAccount := k.authKeeper.GetModuleAccount(ctx, state.evoluteaiRequestsAccountName)
	k.authKeeper.SetModuleAccount(ctx, requestsModuleAccount)
	evoluteaiRewardsModuleAccount := k.authKeeper.GetModuleAccount(ctx, state.evoluteaiRewardsAccountName)
	k.authKeeper.SetModuleAccount(ctx, evoluteaiRewardsModuleAccount)
