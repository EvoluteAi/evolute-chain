...
func (s *KeeperTestSuite) TestMsgRegisterReputerInvalidInsufficientStakeToRegisterAfterRemovingRegistration() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(gomock.Any(), reputerAddr, state.evoluteaiStakingAccountName, registrationInitialStakeCoins)
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, reputerAddr, registrationInitialStakeCoins.QuoInt(cosmosMath.NewInt(2)))
...
func (s *KeeperTestSuite) TestMsgRegisterReputerAddAndRemoveAdditionalTopic() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(
		gomock.Any(),
		reputerAddr,
		state.evoluteaiStakingAccountName,
		sdk.NewCoins(
			sdk.NewCoin(
				params.DefaultBondDenom,
...
func (s *KeeperTestSuite) commonStakingSetup(ctx sdk.Context, reputerAddr sdk.AccAddress, workerAddr sdk.AccAddress, registrationInitialStake cosmosMath.Uint) {
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(gomock.Any(), workerAddr, state.evoluteaiStakingAccountName, registrationInitialStakeCoins)
...
func (s *KeeperTestSuite) TestMsgAddStake() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(gomock.Any(), reputerAddr, state.evoluteaiStakingAccountName, stakeAmountCoins)
...
func (s *KeeperTestSuite) TestMsgAddAndRemoveStakeWithTargetWorkerRegisteredInMultipleTopics() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, reputerAddr, stakeAmountCoins)
	_, err = msgServer.ConfirmRemoveStake(ctx, &state.MsgConfirmRemoveStake{
		Sender: reputerAddr.String(),
	})
	require.NoError(err, "ConfirmRemoveStake should not return an error")
...
func (s *KeeperTestSuite) TestAddStakeInvalid() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(gomock.Any(), reputerAddr, state.evoluteaiStakingAccountName, stakeAmountZeroCoins)
	_, err := msgServer.AddStake(ctx, &state.MsgAddStake{
		Sender:      reputerAddr.String(),
		StakeTarget: workerAddr.String(),
...
func (s *KeeperTestSuite) TestMsgConfirmRemoveStake() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, reputerAddr, removalAmountCoins)
...
func (s *KeeperTestSuite) TestMsgConfirmRemoveAllStake() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, reputerAddr, registrationInitialStakeCoins)
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, reputerAddr, stakeAmountCoins)
...
func (s *KeeperTestSuite) TestModifyStakeInvalidNotHaveEnoughBond() {
	...
	s.bankKeeper.EXPECT().MintCoins(gomock.Any(), state.evoluteaiStakingAccountName, registrationInitialStakeCoins)
	s.bankKeeper.MintCoins(s.ctx, state.evoluteaiStakingAccountName, registrationInitialStakeCoins)
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, reputerAddr, registrationInitialStakeCoins)
	s.bankKeeper.SendCoinsFromModuleToAccount(s.ctx, state.evoluteaiStakingAccountName, reputerAddr, registrationInitialStakeCoins)
	// Register Reputer
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(gomock.Any(), workerAddr2, state.evoluteaiStakingAccountName, registrationInitialStakeCoins)
...
func (s *KeeperTestSuite) TestRequestInferenceSimple() {
	...
	s.bankKeeper.EXPECT().MintCoins(gomock.Any(), state.evoluteaiStakingAccountName, initialStakeCoins)
	s.bankKeeper.MintCoins(s.ctx, state.evoluteaiStakingAccountName, initialStakeCoins)
	s.bankKeeper.EXPECT().SendCoinsFromModuleToAccount(gomock.Any(), state.evoluteaiStakingAccountName, senderAddr, initialStakeCoins)
	s.bankKeeper.SendCoinsFromModuleToAccount(s.ctx, state.evoluteaiStakingAccountName, senderAddr, initialStakeCoins)
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(s.ctx, senderAddr, state.evoluteaiRequestsAccountName, initialStakeCoins)
...
func (s *KeeperTestSuite) TestRequestInferenceBatchSimple() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(s.ctx, senderAddr, state.evoluteaiRequestsAccountName, requestStakeCoins)
	s.bankKeeper.EXPECT().SendCoinsFromAccountToModule(s.ctx, senderAddr, state.evoluteaiRequestsAccountName, requestStakeCoins)
	response, err := s.msgServer.RequestInference(s.ctx, &r)
	s.Require().NoError(err, "RequestInference should not return an error")
	s.Require().Equal(&state.MsgRequestInferenceResponse{}, response, "RequestInference should return an empty response on success")
