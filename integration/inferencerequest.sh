...
$evoluteaiD_BIN tx emissions request-inference \
  $RI_CREATOR \
...
do
  MEMPOOL=$($evoluteaiD_BIN query emissions all-inference-requests)
  if [ "$MEMPOOL" == "{}" ]; then
...
$evoluteaiD_BIN tx emissions reactivate-topic $RI_CREATOR 1 --yes --gas-prices=1uallo --gas=auto --gas-adjustment=1.5;
sleep 3

echo "Checking that the evoluteai daemon is still running"
PSAUX=$(ps aux | grep evoluteaid | wc -l)
if [[ "$PSAUX" != "2" ]]; then
  echo "The evoluteai daemon is not running"
  exit 1
fi
